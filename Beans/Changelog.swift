...
enum ChangelogStore {
    ...
    static let logs: [VersionLog] = [
        VersionLog(
            id: "1.5.0",
            version: "1.5.0",
            title: "同步上游 1.5.7，音源改用原作者最新实现",
            features: [
                "音源子系统整体切换到原作者最新实现（内置音源调用池扩容，缓解 VIP 解析失败）",
                "新增控制中心封面页（液态玻璃）与封面渐变、高光自定义",
                "播放页背景浮尘开关、壁纸自动同步播放器与控制中心背景",
                "播放页支持顶部下划关闭，缩放淡出动画",
                "强制全局 120Hz 高刷新，修复多页面 60Hz 限制",
                "修复 QQ 微信登录同步、长按歌曲下一首/立即播放误触发等问题",
            ],
            fixes: [
                "保留封面共享缓存、缩略图、进度节流、防抖写入等全部优化",
                "保留品牌自定义（图标、名称、更新检测）与播放键重试修复",
            ]
        ),
        VersionLog(
            id: "1.4.1",
            version: "1.4.1",
            title: "音源系统回退到 1.2.0 稳定实现",
            features: [
                "音源子系统整体回退：解灰服务、音源存储、导入页、LX 运行时与 1.2.0 完全一致",
                "恢复内置音源（QQ 稳定源 / 网易云统一源）+ 用户导入音源 + LX 脚本三层解析",
                "移除上游 1.5.5 的收费配额源架构，不再受每日 1000 次共享配额限制",
                "保留 1.5.5 其余改进与本项目全部性能优化、播放键重试修复",
            ],
            fixes: [
                "修复 1.3.x / 1.4.0 播放异常与卡歌问题（回退到实测稳定的 1.2.0 音源实现）",
            ]
        ),
        VersionLog(
            id: "1.4.0",
